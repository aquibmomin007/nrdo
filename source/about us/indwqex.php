
<?php 
    set_include_path($_SERVER['DOCUMENT_ROOT'] . '/includes');
    $primary = 1;
    include('header.php');
    

?>

     --><div class="link-description">
       
        
            <div class="wrapper">
                    <h1>Welcome to National Registry of Diseases Office</h1>
                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed et fringilla dolor. Ut mattis posuere nibh, vitae consectetur sem laoreet at. </p>
                    <div class="main-links">
                                <ul class="link-list bxslider">
                                    <div>
                                        <li  ><a href="#" class="ami"></a></li>
                                        <li><a href="#" class="cancer"></a></li>
                                        <li><a href="#" class="kidney"></a></li>
                                        <li><a href="#" class="stroke"></a></li>
                                        <li class="mobile-center"><a href="#" class="trauma"></a></li>
                                    </div>
                                    <div>    
                                        <li><a href="#" class="donor"></a></li>
                                        <li><a href="#" class="birth"></a></li>
                                    </div>    

                                </ul>  


                                
                    </div> 
                     
            </div>
        </div>            


                    <div class="container">
                            <div class="wrapper main-body">
                                    <section class="right-container">
                                            <article class="side-content">
                                                <img src="/assets/img/data-e.png" alt="Data Enquiry">
                                                <div class="side-data">
                                                    <h3 class="side-heading">Data Enquiry Form</h3>
                                                    <p>Vivamus condimentum gravida orci, a tinci dunt mau ris hendre rit at. </p>
                                                    <span class="side-button"> <a href="#">Download</a></span>
                                                </div>
                                            </article>
                                            <article class="side-content">
                                                <img src="/assets/img/data-r.png" alt="Data Request">
                                                <div class="side-data">
                                                    <h3 class="side-heading">Check Data Request Status</h3>
                                                    <p>Vivamus condimentum gravida orci, a tinci dunt mau ris hendre rit at. </p>
                                                    <div class="track"><input type="text" placeholder="your tracking id" /><a href="#"></a></div> 
                                                </div>
                                            </article>
                                            <article class="side-content">
                                                <img src="/assets/img/disease-n.png" alt="Disease Notification">
                                                <div class="side-data">
                                                    <h3 class="side-heading">Disease <br />e-Notification Login.</h3>
                                                    <p>Vivamus condimentum gravida orci, a tinci dunt mau ris hendre rit at. </p>
                                                    <span class="side-button"><a href="#">Login Now</a></span>
                                                </div>
                                            </article>
                                            <article class="side-content">
                                                <img src="/assets/img/health-p.png" alt="Health Portal">
                                                <div class="side-data">
                                                    <h3 class="side-heading">Health professional portal login.</h3>
                                                    <p>Vivamus condimentum gravida orci, a tinci dunt mau ris hendre rit at. </p>
                                                    <span class="side-button"><a href="#">Login Now</a></span>
                                                </div>

                                            </article>
                                    </section>

                                    <section class="left-container">
                                        <h2><span>Latest</span> Publications</h2>
                                       <article class="publication">
                                                <h3>Annual reports on AMI from 2007 to 2010</h3>
                                                    <p>Vivamus condimentum gravida orci, a tincidunt mauris hendre rit at. Aenean ipsum eros, euis mod quis augue sed.</p>
                                                    <a href="#">Read More <img src="/assets/img/arrow-publi.png" alt="arrow" /></a>
                                       </article>
                                       
                                       <article class="publication">
                                                <h3>Annual reports on Stroke from 2007 to 2011</h3>
                                                <p>Vivamus condimentum gravida orci, a tincidunt mauris hendre rit at. Aenean ipsum eros, euis mod quis augue sed.</p>
                                                <a href="#">Read More <img src="/assets/img/arrow-publi.png" alt="arrow" /></a>
                                       </article> 
                                       
                                       <article class="publication">
                                                <h3>Cancer Incidence in Singapore 1993–1997 (Report No. 5) </h3>
                                                <p>Vivamus condimentum gravida orci, a tincidunt mauris hendre rit at. Aenean ipsum eros, euis mod quis augue sed.</p>
                                                <a href="#">Read More <img src="/assets/img/arrow-publi.png" alt="arrow" /></a>
                                       </article> 

                                       <article class="publication">
                                                <h3>Report of the Singapore Renal Registry 1997 </h3>
                                                <p>Vivamus condimentum gravida orci, a tincidunt mauris hendre rit at. Aenean ipsum eros, euis mod quis augue sed.</p>
                                                <a href="#">Read More <img src="/assets/img/arrow-publi.png" alt="arrow" /></a>
                                       </article> 

                                       <article class="publication">
                                                <h3>Trends of Colorectal Cancer in Singapore 2007-2011</h3>
                                                <p>Vivamus condimentum gravida orci, a tincidunt mauris hendre rit at. Aenean ipsum eros, euis mod quis augue sed.</p>
                                                <a href="#">Read More <img src="/assets/img/arrow-publi.png" alt="arrow" /></a>
                                       </article> 

                                       <article class="publication">
                                                <h3>Annual reports on AMI from 2007 to 2010</h3>
                                                <p>Vivamus condimentum gravida orci, a tincidunt mauris hendre rit at. Aenean ipsum eros, euis mod quis augue sed.</p>
                                                <a href="#">Read More <img src="/assets/img/arrow-publi.png" alt="arrow" /></a>
                                       </article>

                                       <div class="view-more"><a href="#">View More Publications<img src="/assets/img/arrow-more.png" alt="arrow-more" /></a></div> 

                                    </section>   

                                    <div class="clearfix"></div>  
                                       
                            </div>
                    </div>    
             

<?php 
     include('footer.php');
?>
      

